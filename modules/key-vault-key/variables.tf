...
variable "purge_protection_enabled" {
  description = "Block permanent deletion until the soft-delete window expires. IRREVERSIBLE — Azure does not allow turning this off once a vault has it, so a destroyed vault cannot be purged early and its globally unique name stays reserved for soft_delete_retention_days. Leave true for any vault holding a customer-managed key: purging a key that encrypts live data makes that data unrecoverable, and several Azure services require purge protection before they will accept a CMK. Set false only for throwaway vaults you expect to destroy and recreate under the same name."
  type        = bool
  default     = true
}
