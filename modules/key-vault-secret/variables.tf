...
variable "secrets" {
  ...
  # The mirror check. An orphaned value is almost always a typo in one of the
  # two maps — silently ignoring it would create the wrong secret and skip the
  # intended one.
  #
  # It lives here, on secrets, rather than on secret_values where it reads more
  # naturally. Terraform rejects circular references between variable
  # validations, so secrets validating secret_values AND secret_values
  # validating secrets fails to load with "Cycle: var.secret_values
  # (validation), var.secrets (validation)". Only one variable may point at the
  # other, so both directions are checked from this side.
  validation {
    condition = alltrue([
...
variable "secret_values" {
  ...
  type        = map(string)
  sensitive   = true
  # No validation block here: both key-matching checks live on var.secrets,
  # because Terraform rejects two variables whose validations reference each
  # other.
}
